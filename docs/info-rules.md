...
Project_Model == Node_Build_Tooling_default
...
Env == Node_Build_Tooling
Package_Manager == npm_preferred
Dependency_Update_Rule => prefer semver_compatible_parent_updates over transitive_lockfile_surgery
Watcher_Rule => preserve chokidar_driven_scss_watch behavior under `npm run dev:hot-reload`
Build_Rule => preserve `npm run build` output contract for css and docs artifacts
Verification_Rule => require build_success + watcher_change_detected after tooling_dependency_updates

[10_Review_Gate]
Pass_Condition == Headers_match && Atomic_bullets && No_tutorial_content && Explicit_boundaries && File_parseable_by_LLM && Tooling_contract_preserved
Interpret_Rule == Strict_SLM_format > Natural_prose
